...
#LD_PRELOAD=/usr/lib/arm-linux-gnueabihf/libatomic.so.1.2.0 python3 camera2.py
...
camera = PiCamera()
camera.resolution = (800, 385) #a smaller resolution means faster processing
camera.framerate = 24
rawCapture = PiRGBArray(camera, size=(800, 385))

img = Image.open('/home/pi/music2/bg_overlay.png')
	...
	if len(contours) !=0:
		...
		for x in contours:
			...
			if area > threshold_area:
				...
				if ((y+h) > yAlert):
					cv2.putText(image, "ALERT!", (centerX -20, yAlert +20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255,255,255),2)
					GPIO.output(buzzer, True)
					#try:
						#_thread.start_new_thread( playSound, ("Thread-1") )
					#except:
